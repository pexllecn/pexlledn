      ...
      <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-4">
          <Surface className="p-5">
            <div className="flex items-center justify-between">
              <p className="text-[15px] font-semibold text-foreground">July 2026</p>
              <Stepper label="Jul" />
            </div>
            <div className="mt-4 grid grid-cols-7 gap-1 text-center">
              ...
              {MINI.map((n) => {
                ...
                return (
                  <span
                    key={n}
                    className={`flex h-8 items-center justify-center rounded-full text-[12px] tabular-nums ${
                      n === 5
                        ? "bg-[#0A6CFF] font-semibold text-white"
                        : valid
                        ? "text-foreground"
        ...
        <Surface className="overflow-hidden p-0">
          <div className="flex items-center justify-between px-5 py-4">
            <div>
              <p className="text-[13px] text-muted-foreground">Week of</p>
              <p className="mt-0.5 text-[20px] font-semibold tracking-[-0.02em] text-foreground">
                29 Jun – 5 Jul
              </p>
            </div>
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[720px]">
              {/* Day header */}
              <div className="grid grid-cols-[56px_repeat(7,minmax(0,1fr))] border-b border-black/[0.06] dark:border-white/[0.07]">
                <span />
                {DAYS.map((d, i) => (
                  <div key={d} className="px-2 py-3 text-center">
                    ...
                    <p
                      className={`mx-auto mt-1 flex h-7 w-7 items-center justify-center rounded-full text-[13px] font-semibold tabular-nums ${
                        i === 6
              ...
              <div className="relative grid grid-cols-[56px_repeat(7,minmax(0,1fr))]">
                <div>
                  {HOURS.map((h) => (
                    <div
                      key={h}
                      className="relative pr-2 text-right text-[11px] tabular-nums text-muted-foreground"
                      style={{ height: ROW }}
                    >
                ...
                {DAYS.map((d, dayIndex) => (
                  <div
                    key={d}
                    className="relative border-l border-black/[0.05] dark:border-white/[0.06]"
                  >
                    {HOURS.map((h) => (
                      <div
                        key={h}
          ...
          <div className="flex flex-wrap items-center gap-4 px-5 py-4">
            {calendars.slice(0, 3).map((c) => (
              <span key={c.label} className="flex items-center gap-1.5 text-[13px]">
                <span
