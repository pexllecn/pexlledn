    ...
    <MediaPage title="Photos">
        ...
        <div className="flex items-center gap-2">
          <Tabs defaultValue="all">
            <TabsList className="rounded-full">
              ...
              <TabsTrigger value="fav" className="rounded-full">
                Favorites
              </TabsTrigger>
              <TabsTrigger value="rec" className="rounded-full">
                Recents
              </TabsTrigger>
            </TabsList>
          </Tabs>
          <Button className="rounded-full bg-gradient-to-r from-sky-500 to-cyan-400 text-white border-none shadow-lg shadow-cyan-500/25 hover:opacity-90">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {albums.map((a) => (
          <div
            key={a.name}
            className={`group relative overflow-hidden rounded-3xl border border-border/60 cursor-pointer ${glassHover}`}
          >
      ...
      {/* Memory banner */}
      <div
        className={`${glass} relative overflow-hidden flex items-center gap-4 p-5`}
      >
        <div className="pointer-events-none absolute -left-10 -top-10 h-40 w-40 rounded-full bg-sky-500/20 blur-3xl" />
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-500 to-cyan-400 text-white shadow-lg">
        ...
        <Button
          variant="outline"
          size="sm"
          className="shrink-0 rounded-full bg-background/50 backdrop-blur-md"
        >
          View memory
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 auto-rows-[150px] gap-3">
        {photos.map((p, i) => (
          ...
          >
            ...
            {p.liked && (
              <span className="absolute top-2.5 right-2.5 flex h-7 w-7 items-center justify-center rounded-full bg-black/30 backdrop-blur-md">
      ...
      <AnimatePresence>
        {active !== null && (
          ...
          >
            <button
              className="absolute top-4 right-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-md hover:bg-white/20"
              onClick={close}
            >
              <X className="h-5 w-5" />
            </button>
            <button
              className="absolute left-4 flex h-11 w-11 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-md hover:bg-white/20"
              onClick={(e) => {
                e.stopPropagation();
                step(-1);
              }}
            ...
            <button
              className="absolute right-4 flex h-11 w-11 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-md hover:bg-white/20"
              onClick={(e) => {
                e.stopPropagation();
                step(1);
              }}
