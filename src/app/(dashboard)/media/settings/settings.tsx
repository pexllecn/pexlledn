  ...
  return (
    <div className={`${card} p-6`}>
      <div className="mb-3 flex items-start gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-muted text-orange-500 dark:bg-white/5">
  ...
  return (
    <Cinema title="Settings">
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          <h2 className="text-3xl font-semibold tracking-tight text-foreground">Media settings</h2>
          <p className="mt-1 text-sm text-muted-foreground">Tune playback, appearance and storage to taste</p>
        </div>
        ...
        <div className={`${card} flex flex-wrap items-center gap-4 p-6`}>
          ...
          <div className="min-w-0 flex-1">
            <p className="text-lg font-semibold text-foreground">Alex Rivera</p>
            <p className="text-sm text-muted-foreground">alex@media.app</p>
          </div>
          <span className="inline-flex items-center gap-1 rounded-full bg-orange-500/15 px-3 py-1 text-xs font-medium text-orange-500">
            <Sparkles className="h-3 w-3" /> Premium
          </span>
          <button className="rounded-full border border-border px-4 py-2 text-sm text-foreground hover:bg-muted dark:hover:bg-white/5">Edit profile</button>
        </div>

        {/* playback */}
        <Panel icon={Play} title="Playback" desc="How your media plays by default">
          <div className="mb-4">
            <p className="mb-2 text-sm font-medium text-foreground">Streaming quality</p>
              ...
              {qualities.map((q) => (
                <button
                  ...
                  className={`rounded-full px-3.5 py-1.5 text-sm transition-colors ${
                    quality === q ? "bg-orange-500 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70 dark:bg-white/5 dark:hover:bg-white/10"
                  }`}
                >
                  {q}
                </button>
          ...
          <div className="mb-4 flex flex-wrap gap-3">
            {accents.map((a) => (
              <button key={a.name} onClick={() => setAccent(a.name)} className="flex flex-col items-center gap-1.5">
                <span className={`h-10 w-10 rounded-full ${a.cls} ${accent === a.name ? "ring-2 ring-foreground ring-offset-2 ring-offset-background" : ""}`} />
        ...
        <Panel icon={HardDrive} title="Storage & data" desc="Manage downloads and network use">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl bg-muted p-3 dark:bg-white/5">
            <Wifi className="h-4 w-4 text-emerald-500" />
            <span className="text-sm text-foreground">Connected · downloading over Wi-Fi</span>
            <button className="ml-auto rounded-full border border-border px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted dark:hover:bg-white/5">Clear cache</button>
          </div>
        </Panel>
        ...
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-dashed border-border p-4">
          ...
          <button className="rounded-full border border-border px-4 py-2 text-sm text-foreground hover:bg-muted dark:hover:bg-white/5">Manage</button>
        </div>

        <div className="flex justify-end gap-2 pb-1">
          <button className="rounded-full px-4 py-2 text-sm text-muted-foreground hover:text-foreground">Reset</button>
